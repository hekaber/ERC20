...
describe("StableCoin", function () {
    ...
    let stableCoinAddress: SignerWithAddress;
    ...
    this.beforeEach(async () => {
        ...
        stableCoinAddress = (await ethers.getSigners())[1];
    });
    ...
    it("Should allow minting", async function () {
        ...
        await StableCoin.connect(stableCoinAddress).mint({
    ...
    describe("With minted tokens", function () {
        ...
        it("Should allow burning", async function () {
            ...
            await StableCoin.connect(stableCoinAddress).burn(
        ...
        it("Should prevent depositing collateral buffer below minimum", async function () {
            ...
            await expect(
                StableCoin.connect(stableCoinAddress).depositCollateralBuffer({
                    value: ethers.utils.parseEther(stableCoinCollateralBuffer.toString()),
                })
            ).to.be.revertedWithCustomError(
                StableCoin,
                "InitialCollateralRatioError"
            ).withArgs("STC: Initial collateral ratio not met, minimum is ", ethers.utils.parseEther(expectedMinimumAmount.toString()));
        });

        it("Should allow depositing collateral buffer", async function () {
            const stableCoinCollateralBuffer = 0.5;
            await StableCoin.connect(stableCoinAddress).depositCollateralBuffer({
                value: ethers.utils.parseEther(stableCoinCollateralBuffer.toString()),
            });
